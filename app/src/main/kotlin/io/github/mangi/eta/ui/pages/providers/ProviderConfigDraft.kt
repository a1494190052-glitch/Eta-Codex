...
internal fun buildUpdatedProvider(
    ...
    // ChatGPT Codex 订阅 provider：强制官方地址与 Responses 端点，API Key 恒为空，
    // 凭据由 CodexAuthRepository 单独保存在 Android Keystore。
    val codexSubscription = source.isCodexSubscription
    val resolvedBaseUrl = if (codexSubscription) CodexSubscription.BASE_URL else baseUrl.trim()
    val resolvedApiKey = if (codexSubscription) "" else apiKey.trim()
    val resolvedEndpointMode = if (codexSubscription) {
    ...
    return when (source) {
        is OpenAiCompatibleProviderSetting -> source.copy(
            ...
            hostedWebSearchEnabled = hostedWebSearchEnabled && !codexSubscription,
        )
