...
internal object OpenAiResponsesProvider : AgentProviderClient {
    ...
    private fun buildHeaders(
        ...
        // 与上游一致：统一注入 User-Agent、用户自定义头与会话路由头（opencode.ai）。
        // Codex 的账号与来源头在其后强制写入，用户自定义头无法覆盖。
        ProviderRequestHeaders.mergeInto(headers, config.baseUrl, config.customHeaders, sessionId)
